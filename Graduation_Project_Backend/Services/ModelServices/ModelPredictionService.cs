...
namespace Services.ModelServices
{
    public class ModelPredictionService : IModelPredictionService
    {
        ...
        public async Task<SavedPredictionResponseDto> CreatePreeclampsiaPredictionAsync(string email, CreatePreeclampsiaPredictionDto request)
        {
            if (string.IsNullOrWhiteSpace(email))
                throw new UnauthorizedException();
            ...
            var modelCall = await PredictPreeclampsiaInternalAsync(request.Data);
            ...
            var predictionRecord = new PredictionRecord
            {
                PatientId = patient.Id,
                DoctorId = doctor.Id,
                Type = PredictionType.Preeclampsia,
                Result = modelResult.Result,
                Confidence = confidence,
                InputJson = inputJson,
                RawResponseJson = modelCall.RawResponseJson,
            ...
            return new SavedPredictionResponseDto
            {
                PredictionRecordId = predictionRecord.Id,
                PatientId = patient.Id,
                Type = PredictionType.Preeclampsia.ToString(),
                Result = predictionRecord.Result,
                Confidence = predictionRecord.Confidence,
                CreatedAt = predictionRecord.CreatedAt
            };
        ...
        private async Task<(PredictionResponseDto Result, string RawResponseJson)> PredictPreeclampsiaInternalAsync(PreeclampsiaPredictionRequestDto request)
        {
            var endpoint = _configuration["ModelApi:PreeclampsiaPredictEndpoint"] ?? "preeclampsia/predict";
