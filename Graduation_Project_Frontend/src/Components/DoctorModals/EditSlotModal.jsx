...
export default function EditSlotModal({ slot, token, onClose, onSaved }) {
  const [startTime, setStartTime] = useState(() => parseStartTime(slot.time));
  const [duration, setDuration]   = useState(() => parseDuration(slot.duration));
  const [type, setType]           = useState(slot.visitType || "Online");
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(
        `https://her-journey-669913381811.us-central1.run.app/api/Doctor/UpdateAvailabilitySlot?SlotId=${slot.id}`,
        {
          method: "PUT",
          headers: {
          ...
          body: JSON.stringify({
            startAt: toISO(slot.date, startTime),
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl border border-gray-100 shadow-2xl w-full max-w-sm overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div>
            <h3 className="text-base font-bold text-gray-900">Edit Slot</h3>
            <p className="text-xs text-gray-400 mt-0.5">{slot.dateLabel}</p>
          </div>
